...
/*Server.prototype.sendServerChanList = function(user) {
	...
	this.sendServerEvt(	user, "chan-list", Object.keys(this.chans));
};*/
...
Server.prototype._parse = function(user, data) {
	...
	if (msg.chanEvt !== undefined) {
		...
		if (d.label !== undefined) {
			...
			this.sendChanEvt(user.chan.users, d.label, d.data);
...
Server.prototype._changeChanName = function(newName, user) {
	...
	if (newName === undefined) {
		...
		if (this.isValidName(newName)) {
			...
			// dispatch to users
			var list = Object.values(this.userListenChansIds);
...
Server.prototype._createChan = function(user, chanName, pass) {
	...
	if (this.isValidName(chanName)) {
		...
		var chan = this.chans[chanName.toLowerCase()] = new Chan(chanName, pass, this.chanId++);
		var server = this;
		chan.onEmpty = function() { server._removeChan(chan); };
		
		this.sendServerEvt(	Object.values(this.userListenChansIds),
						 	"chan-added", chan.data.name);
		return chan;
	}
