...
Server.prototype.sendChanEvt = function(user, label, data) {
	...
	this._sendList(user.chan.users, str);
};
...
Server.prototype.sendError = function(user, id, vars) {
	...
	var str = JSON.stringify({serverEvt: d});
	this._send(user, str);
 ...
 * @param {User} user - Optional, if you would send the user list only to 1 user
 */
Server.prototype.sendChanUserList = function(chan, deep, user) {
	...
	for (key in data) {
		...
		if (key === "name") {
			...
			if (this._changeUserName(data[key], user))
				user.data.name = data[key];
			else
				delete(data[key]);
			
		// Change his role (unathorized)
		} else if (key === "role" && user.data.role !== data.role) {
			
			delete(data[key]);
			this.sendError(user, 302);
		
		// Join a new chan
		} else if (key === "chan" && user.data.chan.name !== data.chan.name) {
			
			if(!this._joinChan(data.chan.name, data.chan.pass, user)) {
...
Server.prototype._close = function (user) {
	...
	console.log(user.data.name + " disconected");
...
Server.prototype._debug = function () {
	...
	for(var key in this.users) {
		...
		console.log("		" + this.users[key].data.name+ ": " + this.users[key].chan.users.length);
	}
...
Server.prototype._joinChan = function(chanName, pass, user) {
	...
	this.sendChanUserList(chan, true, user);
 ...
 */
Server.prototype._initStartChan = function() {
