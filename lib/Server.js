...
Server.prototype._close = function (user) {
	...
	//this.sendServerEvt(user, "user-offline");
...
Server.prototype._changeUserName = function(newName, user) {
	...
	if (newName === undefined) {
		...
		if (newName.match(reg)) {
			...
			//this.sendUserData(user, {name: newName, id: user.data.id}, user.chan.users);
			//this.sendChanMsg(user.chan, oldName + " is now know as " + newName);
