...
function Server() {
	...
	// start chan
	var chanName = config.startChan.name;
...
Server.prototype.add = function(socket) {
	...
	u.name = config.defaultUser.name + (++this.defaultNameID);
	
	console.log(u.name + " connected");
	this.sendServerMsg("You are connected, your name is " + u.name, u);
	
	
	this._joinChan(config.startChan.name, "", u);
...
Server.prototype._changeUserName = function(newName, user) {
	...
	if (newName === undefined) {
		...
		if (newName.match(regexp)) {
			...
			this.sendChanMsg(oldName + " is now know as " + newName);
