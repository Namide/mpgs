...
/**
 * Trace server datas (IP and HOST).
 *
 * @api public
 */
Server.logDatas = function (callBack) {
	...
	Object.keys(ifaces).forEach(function (ifname) {
		...
		ifaces[ifname].forEach(function (iface) {
			...
			if (alias >= 1) {
				HOST = iface.address;
				callBack(ifname + ": " + alias + " " + iface.address);
				
			// this interface has only one ipv4 adress
			} else {
				HOST = iface.address;
				callBack(ifname + ": " + iface.address);
	...
	if (process.env.PORT != undefined)
		callBack("port open: " + process.env.PORT);
		...
		for (key in data) {
			...
			switch (key) {
				...
				case "chan" :
					...
					if (to.data.chan.name !== data.chan.name) {
				...
				case "role" :
					...
					if (by.data.role === "moderator" || by.data.role === "admin") {
						...
						if (to.data.role !== data.role) {
							...
							this.sendError(by, 312, [to.data.name, data.role]);
							delete(data[key]);
						}
...
Server.prototype._createChan = function(user, chanName, pass) {
	...
	if (this.isValidName(chanName)) {
		...
		chan.onNewModerator = function(user) {
			server.sendUserData(user, {role: user.data.role}, user.chan.users);
		};
